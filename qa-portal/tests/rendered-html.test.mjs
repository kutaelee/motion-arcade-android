...
test("renders the active modern UI and physical QA request with truthful build scope", async () => {
  ...
  assert.match(html, /Upper-body reasons \+ game selection · Physical QA 14/);
  assert.match(html, /부분 신체 인식/);
  assert.match(html, /듀얼 낚시 협동 역할/);
  assert.match(html, /Slice 1B–6 UI/);
  assert.match(html, /APK 다운로드/);
  assert.match(html, /2P SAFETY QA/);
  assert.match(visibleText, /활성 QA 요청 5건 보기/);
  assert.match(visibleText, /요청 5건 확인/);
...
test("keeps QA content manifest-driven and ships only the registered review candidate", async () => {
  ...
  assert.match(manifest, /releaseState: "active_prerelease"/);
